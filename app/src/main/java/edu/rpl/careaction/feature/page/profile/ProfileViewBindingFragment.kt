package edu.rpl.careaction.feature.page.profile
...
class ProfileViewBindingFragment : ViewBindingFragment<FragmentMenuProfileBinding>() {
    ...
    private fun initButtonEvent() {
        ...
        binding.btnPrivacyPolicy.setOnClickListener {
            requireParentFragment().requireParentFragment().findNavController()
                .navigate(R.id.action_mainMenuViewBindingFragment_to_privacyPolicyViewBindingFragment)
        }

        binding.btnContact.setOnClickListener {
            val intent = Intent(Intent.ACTION_SENDTO)
            intent.data = Uri.parse("mailto:")
            intent.putExtra(Intent.EXTRA_EMAIL, arrayOf("careaction44@gmail.com"))
    ...
    private fun generateFetchApiCallback(): ApiCallback<User, ErrorResponse> =
        ApiCallback(
            successCallback = {
                ...
                "${it.response.age} ${getString(R.string.txt_view_years_old)}".also { text ->
                    binding.txtViewAge.text = text
                }
    ...
    private fun generateLogoutApiCallback(): ApiCallback<ResponseBody, ErrorResponse> =
        ApiCallback(
            successCallback = {
                DefaultApiCallbackUtility.successCallback()
                requireParentFragment().requireParentFragment().findNavController()
                    .navigate(R.id.landing_nav_graph)
            },
            loadingCallback = {
                DefaultApiCallbackUtility.loadingCallback(parentFragmentManager)
            },
