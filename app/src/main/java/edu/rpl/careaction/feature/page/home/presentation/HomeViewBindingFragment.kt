...
import edu.rpl.careaction.feature.bmi.BmiCalculator
...
class HomeViewBindingFragment : ViewBindingFragment<FragmentMenuHomeBinding>() {
    ...
    override fun onResume() {
        super.onResume()
        homeViewModel.fetch(NewsRecommendedRequest(5))
    }
    ...
    private fun generateApiCallback(recyclerViewAdapters: Pair<NewsCategoryRecyclerViewAdapter, NewsOverviewRecyclerViewAdapter>): ApiCallback<HomeData, ErrorResponse> =
        ApiCallback(
            successCallback = {
                ...
                ("\"" + it.response.motivation.quotes + "\"").also { text ->
                ...
                if (user.weight != null && user.height != null) {
                    ...
                    binding.progressCircular.setProgress(bmi.toInt(), true)
                }
            },
            loadingCallback = {
