        ...
        when (val position = bundle.getInt(bundleKey, defaultValue)) {
            in 0..2 -> {
                binding.txtViewTittle.text =
                    resources.getStringArray(R.array.txt_welcome_title)[position]
                binding.txtViewContent.text =
                    resources.getStringArray(R.array.welcome_content)[position]
            }
            else ->
                requireParentFragment().findNavController().navigate(R.id.action_welcomeViewBindingFragment_to_registerViewBindingFragment)
        }
    }
}